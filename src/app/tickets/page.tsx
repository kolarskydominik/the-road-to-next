...
const TicketPage = () => {
  return (
    <div className="flex flex-1 flex-col gap-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">TicketPage</h2>
      ...
      <ul className="animate-fade-from-top flex flex-1 flex-col items-center gap-y-4">
        {initialTickets.map((ticket) => (
          <li
            key={ticket.id}
            className="w-full max-w-[420px] rounded-xl border border-slate-100 p-4"
          >
            <div>{TICKETS_ICONS[ticket.status]}</div>
            <h3 className="truncate text-xl font-semibold">{ticket.title}</h3>
            <p className="texrt-sm truncate text-slate-500">{ticket.content}</p>
